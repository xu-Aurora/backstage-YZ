...
class App extends Component {
  ...
  componentDidMount() {
    ...
    let  fileList =  []
    if(this.props.argument.pic) {
      fileList = [{
        uid: '-1',
        name: 'xxx.png',
        status: 'done',
        url:`/backstage/upload/download?uuid=${this.props.argument.pic}&viewFlag=1&fileType=jpg&filename=aa`,
      }]
    }
    this.setState({
      fileList,
      pic: this.props.argument.pic
    })
    this.props.form.setFieldsValue({
      name: this.props.argument.name,
   })
  }
...
function mapStateToProps(state, ownProps) {
  return {
    argument: state.groupManage.saveSeslect
  }
}
...
function dispatchToProps(dispatch) {
  return {
    updateGroup(payload = {}) {
      dispatch({type: 'groupManage/updateGroup', payload})
    },
