...
class App extends Component {
  ...
  onSelect (record, e) {
    ...
    this.props.tenantInfo(record.alldata);
    this.setState({
        detailVisible: true
    })
  }
    ...
    if (showScreen) {
      Element = (
        <Row style={{marginTop: '15px'}}>
          ...
          <Col span={5}>
            <FormItem
              label={(<span style={{fontSize: 14}}>身份关系</span>)}
              labelCol={{span: 7}}
              wrapperCol={{span: 15}}>
                <Select 
                  style={{ width: '100%'}} 
                ...
                <Form layout="horizontal" >
                  <Col span={2} >
                    <Button type="primary" 
                      ...
                      style={{ backgroundColor:'#FFF',color:'#1890ff',borderColor:'#1890ff',marginRight:15}}>新增租户</Button>
                    {/* <Button type="primary" 
                      onClick={this.sendShow.bind(this, 'addVisible')} 
...
function dispatchToProps(dispatch) {
  return {
    ...
    tenantInfo(payload = {}) {
      dispatch({
        type: 'proprietorManagement/tenantInfo',
        payload
      })
    },
  }
