...
import {Button,Row,Col,Form,Card,Tabs,Timeline } from 'antd';
...
class App extends Component {
  ...
  //已处理
  showHtml3(content){
    let label = content.serviceEvaluationLabel;
    return (
      <Card style={{marginBottom:10}}>
        <Col span={20}>
          <p style={{fontWeight:700,marginBottom:10}}>当前状态 : { content.serviceStatusName }</p>
          <div style={{display:'flex'}}>订单评价 : &nbsp;
            <div>
              {
                label ? label.split(',').map((item,index) => 
                  <span key={index} style={{border: '1px solid #ccc',padding:"2px 5px",marginRight:10,borderRadius:5}}>
                    {item}
                  </span>
                ) : ''
              }
          ...
          <div style={{marginLeft: 72,marginTop:10}}>{ content.serviceEvaluationContent }</div>
  ...
  showHtml4(content){
    return (
      <Card style={{marginBottom:10}}>
        <Col span={17}>
          <p style={{fontWeight:700,marginBottom:10}}>订单状态 : { content.serviceStatusName }</p>
          <p>取消原因 : { content.closeReason }</p>
  ...
  render() {
    ...
    }else if(content.serviceStatus == '2'){
      orderStatusShow = this.showHtml3(content)
    }else if(content.serviceStatus == '4'){
