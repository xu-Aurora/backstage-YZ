...
class App extends Component {
  ...
  regx (item) {
    ...
    if(item.type == 2) {
      item.buyerList.forEach(item=>{
        if(!item.firstPrice) {
          message.warning('配送区域不能为空');
          return false
        }
        if(!item.firstPriceFreight) {
          message.warning('配送区域不能为空');
          return false
        }
  ...
  areaShow() {
    ...
    this.props.openArea('addVisible')
  }
  //编辑
  onEdit(index){
    this.props.setList({
...
function dispatchToProps(dispatch) {
  return {
    instAdd(payload = {}) {
      dispatch({type: 'dispatchSet/add', payload})
    },
    setList(payload = {}) {
